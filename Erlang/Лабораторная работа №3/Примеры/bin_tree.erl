...
%% Ѕинарное дерево -- либо €вл€етс€ пустым, либо состоит из данных и двух поддеревьев (каждое из которых -- бинарное дерево).

%% “ип tree
-type tree() :: empty | {node, any(), tree(), tree()}.

%%% ƒва возможных варианта интерфейса
%%%%%%%%%%%%%%%%
%%% 1. Ѕолее традиционный
%%%  онструкторы
...
-spec is_tree(any()) -> boolean(). %% провер€ет, €вл€етс€ ли X деревом
is_tree(X) -> is_empty(X) orelse is_node(X).

-spec is_empty(tree()) -> boolean(). %% провер€ет, пустое ли дерево
is_empty(empty) -> true;
is_empty(_) -> false.

-spec is_node(tree()) -> boolean. %% провер€ет, есть ли в дереве данные (можно обойтись и одним из них)
...
%%% 2. Ѕолее удобный в €зыке с сопоставлением
%%%  онструкторы те же
%%% ѕредикаты и селекторы замен€ютс€ одной функцией
...
%% «аметьте, что каждый из этих интерфейсов легко реализуетс€ через другой